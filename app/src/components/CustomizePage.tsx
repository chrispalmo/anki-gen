...
export const CustomizePage: React.FC<{
  ...
  const handleTextChange = useCallback(
    (index: number, field: "original" | "pinyin" | "extra", value: string) => {
  ...
  const generateAndDownloadCSV = (
    ...
    const csv = Papa.unparse(
      phrases.map((phrase) => {
        const back = `${phrase.pinyin}${phrase.extra ? "\n\n" : ""}${
          phrase.extra
        }`; // append extra with two new lines if it's not empty
        return [phrase.original || '""', back || '""'];
      })
    );
    ...
    link.setAttribute(
      ...
      `${new Date()
        .toISOString()
        .slice(0, 10)}--new-flashcards.anki${cardType}.csv`
    );
  ...
  const handleRefresh = (index: number) => {
    const newPhrases = [...phrases];
    newPhrases[index].pinyin = translateChineseToPinyin(
      newPhrases[index].original
    );
    setPhrases(newPhrases);
  };
              ...
              >
                ...
                >
                  <div style={{ display: "flex", alignItems: "baseline" }}>
                    <EditableField
                      ...
                      onChange={(value) =>
                        handleTextChange(index, "original", value)
                      }
                ...
                >
                  <div style={{ display: "flex", alignItems: "baseline" }}>
                    <EditableField
                      value={phrase.pinyin}
                      onChange={(value) =>
                        handleTextChange(index, "pinyin", value)
                      }
                      onSave={savePhrasesToLocalStorage}
                    />
                    <div
                ...
                >
                  <EditableField
                    value={phrase.extra}
                    onChange={(value) =>
                      handleTextChange(index, "extra", value)
                    }
                    onSave={savePhrasesToLocalStorage}
                  />
                </td>
